        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white flex flex-col gap-y-5 dark:bg-gray-800 p-10 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="item-card flex flex-row justify-between items-center">
                    <div class="flex flex-row items-center gap-x-3">
                        <div>
                            ...
                            <h3 class="text-xl font-bold text-indigo-950">Rp. {{ $productTransaction->total_amount }}</h3>
                        </div>
                    </div>
                    <div class="flex flex-row items-center gap-x-3">
                        <div>
                ...
                <div class="grid-cols-4 grid gap-x-10">
                    <div class="flex flex-col gap-y-5 col-span-2">
                        @forelse ($productTransaction->transactionDetails as $detail)
                            <div class="item-card flex flex-row justify-between items-center">
                                <div class="flex flex-row items-center gap-x-3">
                                    <img src="{{ Storage::url($detail->product->photo ) }}" class="w-[50px] h-[50px]">
                                    <div>
                                        <h3 class="text-xl font-bold text-indigo-950">{{ $detail->product->name }}</h3>
                        ...
                        <div class="item-card flex flex-row justify-between items-center">
                            ...
                            <h3 class="text-xl font-bold text-indigo-950">{{ $productTransaction->address }}</h3>
                        </div>

                        <div class="item-card flex flex-row justify-between items-center">
                            <p class="text-base text-slate-500">City</p>
                            <h3 class="text-xl font-bold text-indigo-950">{{ $productTransaction->city }}</h3>
                        </div>

                        <div class="item-card flex flex-row justify-between items-center">
                            <p class="text-base text-slate-500">Post Code</p>
                            <h3 class="text-xl font-bold text-indigo-950">{{ $productTransaction->post_code }}</h3>
                        </div>

                        <div class="item-card flex flex-row justify-between items-center">
                            <p class="text-base text-slate-500">Phone Number</p>
                            <h3 class="text-xl font-bold text-indigo-950">{{ $productTransaction->phone_number }}</h3>
                        </div>

                        <div class="item-card flex flex-col items-start">
                            <p class="text-base text-slate-500">Notes</p>
                            <h3 class="text-lg font-bold text-indigo-950">{{ $productTransaction->notes }}</h3>
                        </div>
                ...
                <div class="flex flex-row items-center gap-x-1">
                    ...
                    @role('owner')
                        @if ($productTransaction->is_paid)
                            <a href="#" class="font-bold py-3 px-5 rounded-full text-white bg-green-600">Contact Customer</a>
                        @else
                    ...
                    @role('buyer')
                    <a href="#" class="w-fit font-bold py-3 px-5 rounded-full text-white bg-green-600">Contact Admin</a>
                    @endrole
                    <a href="{{ route('product_transactions.index') }}" class="font-bold py-3 px-5 rounded-full text-white bg-red-700">Kembali</a>
                </div>
